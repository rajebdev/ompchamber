...
import { findLiveInstance } from '../runtime.js';
import { getLogFilePath } from '../paths.js';
import { log, printJson, fail, isJson, isQuiet } from '../output.js';
...
export async function run(options) {
  ...
  const live = findLiveInstance(requested);
  ...
  if (json) {
    printJson({ logFile, lines: splitLines(fs.readFileSync(logFile, 'utf8')).slice(-lineCount) });
    return;
  }
  ...
  writeLines(splitLines(fs.readFileSync(logFile, 'utf8')).slice(-lineCount));
}
