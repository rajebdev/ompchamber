...
import { listRegistries, findLiveInstance, isProcessAlive, stopInstance } from '../runtime.js';
import { STOP_TIMEOUT_MS } from '../process-lifecycle.js';
import { ok, warn, printJson, isJson } from '../output.js';
...
function resolvePort(options) {
  ...
  const env = process.env.OMPCHAMBER_PORT;
...
export async function run(options) {
  ...
  let targets;
  if (options?.all) {
    targets = listRegistries().filter((entry) => entry && isProcessAlive(Number(entry.pid)));
  } else {
    const live = findLiveInstance(explicitPort(options));
    targets = live ? [live] : [];
  }
