...
export function StatCard({ value, label, hint, tone = 'default' }: StatCardProps) {
  const shell = tone === 'muted' ? 'bg-ink/5 border-ink/10' : 'bg-paper border-ink/15';
  return (
    <div className={`${shell} border rounded-lg p-3 shadow-xs flex flex-col justify-between h-full`}>
      <div>
        <div className="text-base font-bold tracking-tight text-ink leading-tight break-words">{value}</div>
