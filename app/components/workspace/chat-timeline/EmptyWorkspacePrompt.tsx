      ...
      <div className="w-full max-w-3xl flex flex-col items-start space-y-2">
        ...
        <div className="relative" ref={workspaceRef}>
          <button 
            ...
            className="flex items-center space-x-1.5 hover:bg-ink/5 px-2 py-1 rounded transition-colors text-xs text-ink/80 font-medium bg-paper border border-ink/20 shadow-sm focus:border-ink/50 outline-none"
          >
            <Folder size={12} className="text-ink/60" />
          ...
          {showWorkspace && (
            <div className="absolute bottom-full left-0 mb-1 w-64 bg-paper border border-ink/20 rounded-md shadow-lg z-50 flex flex-col overflow-hidden text-xs">
              <div className="px-3 py-2 text-[10px] uppercase tracking-wider text-ink/40 font-semibold border-b border-ink/10 bg-canvas">
                Target Workspace
              </div>
                ...
                {folders.length === 0 ? (
                  ...
                  folders.map((f: any) => (
                    ...
                    >
                      ...
                      {selectedFolderId === f.id && <Check size={12} className="text-ink" />}
                    </button>
                  ))
                )}
              </div>
