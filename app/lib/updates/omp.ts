...
import { invalidateOmpCliCache, resolveOmpBin } from '@/lib/omp/core/cli';
import type { UpdateTargetInfo } from '@/types/updates';
...
export async function checkOmpUpdate(): Promise<UpdateTargetInfo> {
  ...
  try {
    const { stdout, stderr } = await execFileAsync(bin, ['update', '--check'], {
      timeout: 30000,
      maxBuffer: 1024 * 1024,
      env: { ...process.env, PAGER: 'cat', FORCE_COLOR: '0' },
    });
