...
import type { BtwModel, BtwTopic, BtwTurn, BtwTurnStatus } from '@/shared/types';
...
export async function createBtwTopic(input: NewBtwTopicInput): Promise<BtwTopic> {
  ...
  await db.run(
    `INSERT INTO btw_topics
       (id, session_id, title, provider, model_id, model_name, leaf_id, promoted_session_id, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)`,
    [row.id, row.session_id, row.title, row.provider, row.model_id, row.model_name, row.leaf_id, now, now],
  );
...
/** Patch one turn's answer and/or status. */
export async function updateBtwTurn(
  topicId: string,
  turnIndex: number,
  patch: { answer?: string; status?: BtwTurnStatus },
): Promise<void> {
  const db = await getDb();
