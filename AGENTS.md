...
- `/btw [question]` typed in the main composer opens a **floating side-question panel** above the docked composer (`src/client/components/workspace/btw-panel/`, mounted from `ComposerDock`, opened by the `omp:btw` window event); bare `/btw` opens the panel's history. The panel layout is two stacked cards — header (`⟳` new topic, `⌄` topic history, `⧉` promote into chat, `✕` close), a scrollable turn list, then the panel's own composer for follow-ups.
