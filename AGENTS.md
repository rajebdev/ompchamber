...
- **Session auto-titling is ONE-SHOT per conversation** (`src/server/lib/omp/session/auto-title.server.ts`, fired from the `agent_end` branch of `src/server/lib/omp/rpc/frame-fold.ts`). omp force-sets `PI_NO_TITLE=1` under `--mode rpc-ui`, so the chamber asks for the title itself by sending `/rename` to the child once the FIRST run settles. Never fire it again: omp derives that title from the NEWEST turns and `generateRenameTitle` reserves a fresh title revision (cancelling any generation still in flight), so a second attempt after turn two names the session after whatever the conversation had become by then — the bug this gate exists to prevent. The latch is `AgentSessionWrapper.autoTitlePending`, seeded in `applyIdentity` from omp's own `messageCount` (a `--resume` child reports the messages it restored, so an existing session is ineligible from the start) and consumed synchronously on entry, before any await. A first message omp's low-signal filter rejects ("hi") therefore leaves the session unnamed for good — the honest outcome, since omp's own titler declines to name it too.

### Side Questions (BTW panel)
- `/btw [question]` typed in the main composer **replaces the composer with the side-question form** (`src/client/components/workspace/btw-panel/`, swapped in by `ComposerDock`, opened by the `omp:btw` window event; the mode state machine is `src/client/hooks/chat/btw/mode.ts`); bare `/btw` opens it on the current history. The form is stacked cards: the topic's turns (rendered only once there is something to show, or an error to explain), the ask card (`⟳` start over, `⌄` topic history, `⧉` promote into chat, `✕` leave the form) whose Enter asks a question of its own, then the side session's composer for follow-ups with attachments and the model chip.
...
- **Context arrives by transcript reuse, never by re-prompting it as text.** A topic owns a private copy of the parent session file (`<db dir>/btw/<sessionId>/<topicId>/session.jsonl`) taken on its first question; the side child is spawned as `omp --mode rpc-ui --resume <that copy> --no-tools` (`src/server/lib/btw/runtime.server.ts`). The parent's prompt-cache prefix survives, the parent file is never written (verified: byte-identical after many questions), and a follow-up after the child was reclaimed resumes the copy and still sees every earlier turn. `--no-tools` mirrors the TUI's "NEVER use tools" and means no approval mode is needed; the `<btw>` prompt template is byte-identical to omp's.
...
- **State and streaming**: `btw_topics`/`btw_turns` in the chamber DB (schema in `src/server/lib/btw/schema.server.ts`); the server is the source of truth and every frame is a snapshot (`btw_state`) or a delta (`btw_delta`). Commands: `POST /api/btw/:sessionId` (`ask`/`abort`/`promote`/`delete`), stream: `GET /api/btw/:sessionId/events` + WS `/:sessionId/ws`. Unlike the agent stream this one is NOT observer-only: history must be readable while no omp child is running, so reading the state also repairs `running` rows whose runtime is gone.
