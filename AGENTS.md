...
- **Editor & Diff toolbars**: every action is an icon-only `<button>` carrying both a `title` and an `aria-label` (a bare `<svg onClick>` is unreachable by keyboard), and each reflects its own state — word wrap / preview / full screen are `aria-pressed`, Save shows `saving` → `saved` → `error`, and Copy flashes a check for 1.5s. Two rules the toolbars exist to enforce: **a failed read is reported, never substituted** (`useFileEditor` has no `fallback`; the mock sample it used to drop in was written straight over the real file by the next save), and **a failed save says so** (`reportSaveError`, since a silent failure left the user believing the file was written). A file whose read failed also has Save disabled — its buffer is empty, so writing it would blank the file on disk. The save status is reset when the active tab changes, because it describes the file that was written.
- **Diff Panel toolbar**: `Discard` posts `actionType: 'revert'` — the server's own name for "restore this file from the index", and the one that handles the untracked case (removes the file). Sending `discard`, which no handler matched, returned `{success: true}` for doing nothing. Stage/unstage/discard await the response and surface a rejection in an inline error strip; the previous fire-and-forget submit flipped the badge optimistically and let a rejected action look successful. The Split/Unified segments each **set** their mode (toggling meant clicking the already-active segment flipped the view). The toolbar is an `@container`, so its labels collapse on a **narrow panel** rather than a narrow viewport — the editor panel is 320px wide on a 1440px screen, and `sm:` kept every label on, overflowing it. The `+n/-n` counts sit in the action strip rather than the identity block: the editor tab carries the file name but never the counts.
- **Diff→editor conversion** goes through `useFileTabs().convertDiffToEditor`, which replaces the tab immutably and reuses `fileTabId` (`src/shared/lib/workspace/file-tab-id.ts`). The panel used to assign `activeFile.isDiff = false` on the object inside `layout.openedFiles`: React saw no change (identical array identity), and because that array is what gets persisted the mutation reached the database — the "diff" tab came back as a file tab after a reload and its diff never returned. If the target file already has a tab the diff entry is dropped instead, or the two would share an id.
