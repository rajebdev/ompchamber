...
- **`node:os`** — `Bun.os` is `undefined`. `os.homedir()` also resolves `USERPROFILE` on Windows, which `Bun.env.HOME` does not. (The CLI uses `Bun.env.HOME` via `path-utils.js` — acceptable there because the CLI is Bun-only tooling; server code keeps `os.homedir()`.) Signal numbers use the local table in `src/cli/lib/process-lifecycle.js` instead of `os.constants.signals`.
